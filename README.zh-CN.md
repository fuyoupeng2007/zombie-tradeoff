# 🧟 打僵尸 · 取舍之战 (Zombie Tradeoff)

> **"捡武器，还是打僵尸？这就是取舍。"**

一款 2D 无尽模式的网页打僵尸小游戏：**左边掉武器卡，右边来僵尸**——去捡武器就顾不上打僵尸，这就是取舍。

## 🎮 玩法

- **核心循环**：左边天上掉武器卡（攻击+1、攻速+1、回血、护盾），右边不断涌来僵尸。你需要左右移动，在"捡武器强化"和"防守僵尸"之间做取舍。
- **无尽模式**：你的成长没有上限（攻击、攻速无限叠加），僵尸的强度也没有上限（血量、速度、数量随存活时间无限增长）——前期碾压，后期被淹没，看你能撑几分钟。
- **稀有道具**（独立计时掉落）：
  - 💣 **清屏弹**（约 30~35 秒一个）：捡到立即全屏爆炸，清空所有僵尸
  - 🚀 **穿甲弹**（约 20~25 秒一个）：从你的枪口发射贯穿光束，穿透路径上所有僵尸
  - 危险时刻（残血 / 被围 / 后期）掉落计时加速，且直接掉在你脚边救命
- **僵尸类型**：普通 / 快速 / 装甲 / 巨力 / 巨型坦克（巨型僵尸血超厚、被咬 −2 ❤）

## 🎮 操作

| 按键 | 功能 |
|---|---|
| `←` `→` / `A` `D` | 左右移动 |
| `空格` / 鼠标左键 | 三连发（自动开火常驻） |
| `Enter` / `空格` | 开始 / 重开 |

## 🖥 运行

双击 `打僵尸.html`（中文版）或 `index.html`（英文版）用浏览器打开即可，或运行本地服务器：

```bash
python -m http.server 8000
# 然后访问 http://localhost:8000/index.html
```

在线试玩（GitHub Pages）：**https://fuyoupeng2007.github.io/zombie-tradeoff/**

## 🛠 技术说明

- **纯原生 HTML + Canvas 2D + JavaScript**，单文件、零依赖、无需构建
- Web Audio 实时合成音效（枪声、爆炸、捡卡、波次号角），无外部音频文件
- 最高分保存在浏览器 localStorage（沙箱/隐私模式下自动降级，不影响运行）
- 无任何外部资源请求，可完全离线运行

## 📦 文件

- `index.html` — 英文版（GitHub Pages 入口）
- `打僵尸.html` — 中文版
- `README.md` — 英文说明
- `README.zh-CN.md` — 本文件（中文说明）

## 📄 许可

本项目代码可自由使用、修改、分发。
